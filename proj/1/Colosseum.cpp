...
Colosseum::Colosseum(int nRows, int nCols): m_history(nRows, nCols)
{
    ...
    {
        std::cout << "***** Colosseum created with invalid size " << nRows << " by "
             << nCols << "!" << std::endl;
        exit(1);
    }
    m_rows = nRows;
...
void Colosseum::display(std::string msg) const
{
    ...
    std::cout << "There are " << villainCount() << " villains remaining." << std::endl;
    if (m_player == NULL)
...
void Colosseum::checkPos(int r, int c) const
{
    if (!isPosInBounds(r, c))
    {
        std::cout << "***** " << "Invalid colosseum position (" << r << ","
             << c << ")" << std::endl;
        exit(1);
    }
}
